...
public class MemberOperator implements SimpleOperator<Member>, MemberOperatorExtension {
    ...
    public int getNumberOfProjectsOf(Member member) {
    ...
    */
    ...
    public void sortByProfitEarnedAscending() {
        for (int i = 0; i < this.memberList.size() - 1; i++) {
            ...
            for (int j = i + 1; j < this.memberList.size(); j++) {
                if (this.getNumberOfProjectsOf(this.memberList.get(j)) < this.getNumberOfProjectsOf(this.memberList.get(minIndex))) {
                    minIndex = j;
                }
            }
    ...
    public void sortByIdAscending() {
        for (int i = 0; i < this.memberList.size() - 1; i++) {
            int minIndex = i;
            for (int j = i + 1; j < this.memberList.size(); j++) {
                if (this.memberList.get(j).getId().compareTo(this.memberList.get(minIndex).getId()) < 0) {
                    minIndex = j;
                }
            }
            Member temp = this.memberList.get(i);
